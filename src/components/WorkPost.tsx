...
export default function WorkPost({title,role_date,description,cta_link}:postProps) {
  ...
  return (
    <article className='flex flex-row flex-1'>
        <figure className='border-2 border-p2 rounded-l-sm'>
            <img className='w-fit h-fit rounded-l-sm' src={placeholder}></img>
        </figure>
        <div className='bg-b1 border-2 border-p2 px-8 py-6 rounded-r-sm flex flex-col flex-1 justify-between w-fit'>
            <div>
            ...
            <Hyperlink icon={<ExternalLinkIcon className="size-6 text-b1 self-center"/>} 
            style='bg-bl1 text-b1 flex-row-reverse w-full'
             href={cta_link} 
            name={t(`works.hyperlink_name`)}/>
        </div>
    </article>
  )
}
